...
const ProductDetailsPage = () => {
  ...
  const getFilteredProducts = () => {
    switch (filter) {
      case 'total':
        return products;
      case 'low_stock':
        // Products with quantity between 1-5 (regardless of stock_status unless explicitly outofstock)
      ...
      case 'out_of_stock':
        // Products explicitly marked as outofstock OR quantity 0 (but not if marked as instock)
        return products.filter(p => {
      ...
      case 'in_stock':
        // Products marked as instock (regardless of quantity) OR products with quantity > 5 and not outofstock
        return products.filter(p => {
  ...
  console.log('ProductDetailsPage filtering debug:');
                  ...
                  {((product.stock_quantity !== null && product.stock_quantity !== undefined && product.stock_quantity <= 5 && product.stock_quantity > 0 && product.stock_status !== 'instock') || 
                    ...
                    <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-md">
                      <p className="text-red-400 font-medium text-sm">
                        ⚠️ {(product.stock_status === 'outofstock' || (product.stock_quantity === 0 && product.stock_status !== 'instock'))
                          ? 'Out of Stock' 
                          : 'Low Stock Alert'}
                      </p>
                      <p className="text-red-400/80 text-xs mt-1">
                        {(product.stock_status === 'outofstock' || (product.stock_quantity === 0 && product.stock_status !== 'instock'))
